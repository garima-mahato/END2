...
class LRFinder(object):
    ...
    def _train_batch(self, iter_wrapper, accumulation_steps):
        ...
        for i in range(accumulation_steps):
            inp1, inp2, num, s = iter_wrapper.get_batch()
            inp1, inp2, num, s = self._move_to_device(inp1, inp2, num, s)

            # Forward pass
            out1, out2 = self.model(inp1, inp2)
            loss = self.criterion(out1, num) + self.criterion(out2, s)
        ...
        pred2 = (F.log_softmax(out2, dim=1)).argmax(dim=1, keepdim=True)
        correct = torch.logical_and(pred1.eq(num.view_as(pred1)), pred2.eq(s.view_as(pred2))).sum().item()
        processed = len(inp1)
        train_acc = (100*correct/processed)

        return total_loss.item(), train_acc

    def _move_to_device(self, inp1, inp2, num, s):
        def move(obj, device):
        ...
        inp1 = move(inp1, self.device)
        inp2 = move(inp2, self.device)
        num = move(num, self.device)
        s = move(s, self.device)
        return inp1, inp2, num, s
    ...
    def _validate(self, dataloader):
        ...
        self.model.eval()
        with torch.no_grad():
            for inp1, inp2, num, s in dataloader:
                # Move data to the correct device
                inp1, inp2, num, s = self._move_to_device(inp1, inp2, num, s)

                if isinstance(inp1, tuple) or isinstance(inp1, list):
                    batch_size = inp1[0].size(0)
                else:
...
class DataLoaderIterWrapper(object):
    ...
    def __next__(self):
        ...
        try:
            inp1, inp2, num, s = next(self._iterator)
        except StopIteration:
            if not self.auto_reset:
                raise
            self._iterator = iter(self.data_loader)
            inp1, inp2, num, s, *_ = next(self._iterator)
